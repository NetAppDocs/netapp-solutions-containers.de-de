---
sidebar: sidebar 
permalink: whats-new.html 
keywords: 'netapp, solutions, containers, what"s new, updates, change log' 
summary: Protokoll der letzten Änderungen am Material zu den NetApp -Containerlösungen. 
---
= Was ist neu bei NetApp Container-Lösungen?
:allow-uri-read: 
:icons: font
:imagesdir: ./media/


[role="lead"]
Erfahren Sie, was es Neues bei Containerlösungen gibt.



[[21-september-2026]]
== 21. September 2026

Die VMware vSphere Kubernetes Service mit NetApp ONTAP Lösung wurde zu den NetApp Containerlösungen hinzugefügt: link:https://docs.netapp.com/us-en/netapp-solutions-containers/vsphere-kubernetes-service/vks-overview.html["vSphere Kubernetes Service-Lösungen"^].



[[18-august-2025]]
== 18. August 2025

Die NetApp Solutions-Site ist jetzt dielink:https://docs.netapp.com/us-en/netapp-solutions-family/index.html["NetApp Lösungsfamilie"^] , das die folgenden Websites umfasst:

* link:https://docs.netapp.com/us-en/netapp-solutions-ai/index.html["NetApp Lösungen für künstliche Intelligenz"^]
* NetApp Container-Lösungen
* link:https://docs.netapp.com/us-en/netapp-solutions-dataops/index.html["NetApp Datenmanagementlösungen"^]
* link:https://docs.netapp.com/us-en/netapp-solutions-databases/index.html["NetApp Datenbanklösungen"^]
* link:https://docs.netapp.com/us-en/netapp-solutions-cloud/index.html["NetApp Public- und Hybrid-Cloud-Lösungen"^]
* link:https://docs.netapp.com/us-en/netapp-solutions-sap/index.html["NetApp Lösungen für SAP"^]
* link:https://docs.netapp.com/us-en/netapp-solutions-virtualization/index.html["NetApp Virtualisierungslösungen"^]

