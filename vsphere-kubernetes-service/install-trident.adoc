...
NOTE: Die Trident Pods werden möglicherweise an diesem Punkt nicht erstellt. Wenn das ReplicaSet-Objekt beschrieben wird, kann ein Fehler bezüglich einer Verletzung von PodSecurity wie unten gezeigt auftreten. Dieser Fehler tritt auf, weil der `trident` Namespace, der während der Installation erstellt wurde, den Kubernetes Pod Security Standard (PSS) mit eingeschränkten Rechten erzwingt. Der Trident Operator benötigt erhöhte Systemberechtigungen zur Verwaltung des Host-Speichers und kann nicht unter den strengen Einschränkungen eines eingeschränkten Profils ausgeführt werden. Zur Behebung erhält der `trident` Namespace ein privilegiertes Profil, sodass der ReplicaSet-Controller die Pods erfolgreich starten kann.
...
----
Die integrierten Kubernetes Pod Security Admission-Labels werden auf den  `trident`Namespace angewendet, um ihn von den Einschränkungen des eingeschränkten Profils auszunehmen:
