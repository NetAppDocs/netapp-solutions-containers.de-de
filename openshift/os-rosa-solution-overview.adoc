...
* link:https://portal.aws.amazon.com/billing/signup["AWS-Konto"^]
...
Verwenden Sie den Parameter `node-prep` und installieren Sie Trident wie gezeigt. Vor dem Ausführen des Installationsbefehls sollte das Installationspaket heruntergeladen worden sein. Siehe link:https://docs.netapp.com/us-en/trident/trident-install/kubernetes-deploy-tridentctl.html#step-1-download-the-trident-installer-package["Das Trident Installationspaket herunterladen"^].
...
Die Trident Backend-Konfiguration legt fest, wie Trident mit dem Speichersystem (in diesem Fall FSx für ONTAP) kommuniziert. Für die Erstellung des Backends werden die Zugangsdaten der Storage Virtual Machine sowie die Cluster Management- und NFS-Datenschnittstellen bereitgestellt. Die link:https://docs.netapp.com/us-en/trident/trident-use/ontap-nas.html["ontap-nas-Treiber"^] wird verwendet, um Speichervolumes im FSx Dateisystem bereitzustellen.
...
----
Sie können die folgenden Befehle ausführen, um eine PVC zu erstellen und zu überprüfen, ob sie erstellt wurde.
